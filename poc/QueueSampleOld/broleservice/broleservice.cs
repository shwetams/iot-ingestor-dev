...
namespace broleservice
{
    ...
    /// </summary>
    internal sealed class broleservice : StatelessService
    {
        public broleservice(StatelessServiceContext context)
            : base(context)
        { }
...
//                long iterations = 0;
                ...
                while (true)
                {
                    ...
                    using (HttpClient httpClient = new HttpClient())
                    {
                        ...
                        ServiceEventSource.Current.ServiceMessage(this.Context, "Processing b role msg recieved -{0}", resp);
                        var msgStr = resp;

                        if (msgStr.Length > 0)
                        {
                            ServiceEventSource.Current.ServiceMessage(this.Context, "Processing b role msg deserialized -{0}", msgStr);
                            //if (msgDet.state != null)
                            // {
                            //     msgDet.state = msgDet.state + " + b_processed";
                            //}
                            //else
                            //{
                            //    msgDet.state = "b_processed";
                            // }
                            ...
                            else
                            {
                                ...
                                string processed_msg = "msg=" + msgStr + " state=" + "b processed";
                            ...
                            if (conn.IsConnected)
                            {
                                ...
                                var cnt = await db.HashIncrementAsync("numMsgesB", 1);
